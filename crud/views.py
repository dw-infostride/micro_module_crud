    ...
    if  request.user.is_authenticated:    # inbuilt method to not let user go to private  pages 
        ...
        if request.method == "POST":
            ...
            if User.objects.filter(email=email).exists():
                ...
                messages.add_message(request, messages.SUCCESS, 'User Created', extra_tags='bg-success text-white')
...
def edit_profile(request):
    ...
    if request.method == "POST":
        ...
        if username != user.username and User.objects.filter(username=username).exists():
            ...
            if profile_pic is not None :   # to not let save changes return a null value to database, hence if pic is not none execute save 
...
#  ___________________________________________________________delete user profile __________________________________________________

def delete_user(request):
    if not request.user.is_authenticated: # if user is not logged in , returned to sign in page
        return redirect('sign-in')
    
    else:
        if request.method == "POST":   # if user enters password , post gets hit
            email = request.user.email # email is get from database
            password = request.POST.get('password') # password is get from the form 
            
            user = authenticate(request, email=email, password=password) # authenticate inbuilt function 
            
            if user is not None:   # if user exists
                id = request.user.id   # id of user is get from database
 
                user = User.objects.get(id=id)  
                user.delete() # deletes user
                messages.add_message(request, messages.SUCCESS, 'Profile Deleted', extra_tags='bg-success text-white')
                return redirect('/sign-up')
                
                
            else : 
                messages.add_message(request, messages.ERROR, 'Invalid Credentials', extra_tags='bg-danger text-white')

                
    
    return render(request, 'delete_user.html') # opens the page on which the above function/view occurs.

# ______________________________________________________________ forgot password view __________________________________________
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if not User.objects.filter(email=email).first():
                messages.add_message(request, messages.ERROR, 'No user found', extra_tags='bg-danger text-white')
                return redirect('/forgot-password')
            
            else:
                
            
                user_obj = User.objects.get(email = email)  # gets user object
                token = str(uuid.uuid4())          # generates token
                profile_obj = User.objects.get(email=user_obj)  # gets email of user
                profile_obj.forgot_password_token = token   # declares the user object token as the token 
                profile_obj.save()
                send_forgot_password_mail(user_obj.email,token)   # calls the function from helpers
                messages.add_message(request, messages.SUCCESS, 'Reset password email sent', extra_tags='bg-success text-white')    
                return redirect('/forgot-password')
...
# _______________________________________________________________________ reset password _____________________________________

#request.user is used to obtain the user object of the currently authenticated user.
# In this case, there is no guarantee that the current user requesting the password reset is
# authenticated or even registered with the site.



#Reset password view:
def reset_password(request,token): # takes parameters request and token
    context = {}
    try:
        user_obj = User.objects.get(forgot_password_token = token ) 
        
        if request.method == 'POST':  # getting to confirm new password
            new_password = request.POST.get('new_password')
            ...
            user_obj.set_password(new_password)   # sets the new password 
            user_obj.save()  # saves the user o bject
            messages.add_message(request, messages.SUCCESS, 'Password changed successfully', extra_tags='bg-success text-white')
            return redirect('/sign-in/')
